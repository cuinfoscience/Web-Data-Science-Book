...
- **Figure 4-2 is not made.** The back-fill plan's Open-Meteo figure (the forecast in Chrome's JSON view) is left out: api.open-meteo.com's robots.txt disallows every path. Chapter 2 treats `robots.txt` as addressed to crawlers rather than API clients, so whether to capture it is the maintainer's decision (`docs/handoff.md`).
