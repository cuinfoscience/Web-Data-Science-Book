...
- **Figure 1-4 is not made.** The back-fill plan's pageviews figure (the chapter's pageviews URL in Chrome's JSON view: `items`, then each day's `timestamp` and `views`) is left out: on 2026-09-24 Wikimedia's REST API answered this cloud session's address with 429 ("You are making too many requests to the API"), on the session's first request and again that evening. The limit counts everyone who shares the address, so capture it from another network. A draft recipe:

  ```yaml
  - id: pageviews-json
    kind: capture
    section: "Calling Your First API"
    url: https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia/all-access/all-agents/University_of_Colorado_Boulder/daily/20260101/20260131
    mode: headed
    window: [816, 600]
    steps: [{wait: {text: 'University_of_Colorado_Boulder'}}]
    crop: {top: 42, width: 800, height: 520}
  ```

  wikimedia.org has no robots.txt (404).
