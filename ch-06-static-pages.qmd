...
Once your parser works for one ceremony, abstract it into a function you can reuse across every year you want. This is exactly the situation `responsible_get()` from @sec-ethics was built for: a named, identified requester, one page at a time, with a pause built in between calls. Here it is again so this chapter's code runs on its own:
...
def responsible_get(url, headers=None, delay=1.0):
    """Make an HTTP GET request with responsible defaults (from @sec-ethics)."""
    default_headers = {
...
With that defined, wrap the parser around it:
