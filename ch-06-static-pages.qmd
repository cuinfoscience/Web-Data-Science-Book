...
Reach for the same class name you used in Strategy 4 there and it fails you in an unexpected way. The raw HTML French Wikipedia sends does carry `class="infobox_v3"` on the infobox table — curl the page and you can see it — but inspect that same table in a live browser after the page finishes loading, and the class is gone: `table.infobox_v3` matches nothing in the rendered DOM. The browser's Inspector, in other words, is not always describing the same HTML your `requests.get()` call receives; something in the page's own JavaScript strips the class after load. What holds up on both sides is the table's `<caption>`, which BeautifulSoup's CSS-selector support can target directly:
