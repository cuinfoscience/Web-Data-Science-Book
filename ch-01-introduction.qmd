...
Anaconda's default `base` environment is fine for your first session, but as you accumulate projects, their library requirements will eventually conflict. The `conda` package manager solves this with *environments*: isolated Python installations, each with its own set of packages. Create one for this book, activate it, and install Jupyter and the core libraries into it:
...
A new environment holds Python and `pip` and little else, not even Jupyter, which is why `notebook` is on the install line. `conda activate webdata` changes the start of your prompt from `(base)` to `(webdata)`, and from then on `pip` installs into `webdata` and the programs you start run from it. Each new terminal starts in `base`, so run `conda activate webdata` whenever you open one to work on this book.
...
In the companion notebook, this code cell sits under a Markdown cell that holds the paragraph above, as @fig-jupyter-cells shows.

![Jupyter's file list in the browser, in Jupyter Notebook 7.6, September 2026. The folder holds this chapter's companion notebook ①: click a name to open it. **New** ② opens a menu whose first item, **Python 3 (ipykernel)** ③, makes a new notebook.](images/ch-01/jupyter-new-menu_annotated.png){#fig-jupyter-new-menu .lightbox fig-alt="Screenshot of Jupyter's file list in a browser, with the Files and Running tabs, and a toolbar with New, Upload, and a refresh button. Marker 1: ch-01-introduction.ipynb, the one file in the folder. Marker 2: the New button, whose menu is open. Marker 3: its first item, Python 3 (ipykernel), above Terminal, Console, New File, and New Folder."}

![The companion notebook in Jupyter, September 2026. A Markdown cell ① holds the chapter's text. Below it, the first code cell has run: its prompt reads `[1]:` ②, and its output ③ is under it. The kernel, Python 3 (ipykernel), is named at the right of the toolbar ④. **Not Trusted**, above it, is Jupyter's label for a notebook you didn't make on this computer; it doesn't stop cells from running.](images/ch-01/jupyter-cells_annotated.png){#fig-jupyter-cells .lightbox fig-alt="Screenshot of the notebook ch-01-introduction in Jupyter, with the menus File to Help, Not Trusted at the right, and a toolbar ending in marker 4, the kernel Python 3 (ipykernel). Marker 1: a Markdown cell, the paragraph beginning A notebook consists of cells. Marker 2: a code cell's prompt, [1]:, beside a comment and a print call for Hello, web data science! Marker 3: its output, Hello, web data science!"}
...
This book uses several Python libraries extensively. You installed the core ones into `webdata` along with Jupyter; later chapters install more as they need them, with the same `pip install` in a terminal where `webdata` is active. For now, confirm in a notebook that the core libraries import without error:
...
This is raw HTML — not yet useful for analysis. It is the page before a browser draws it, as @fig-article-view-source shows side by side. In @sec-data-formats and @sec-static-pages, you will learn to parse this HTML into structured data using BeautifulSoup. For now, the point is that retrieving a web page programmatically is as simple as `requests.get(url)`.

![The chapter's Wikipedia article, September 2026. On the left, the page as Chrome draws it. On the right, the same address in Chrome's View Source (Ctrl+U; Option-Command-U on a Mac): the HTML the server sends, which `requests.get()` returns as `response.text`. Line 5's `<title>` is the heading on the left. The source's long lines run past the right edge.](images/ch-01/article-view-source.png){#fig-article-view-source .lightbox fig-alt="Two screenshots side by side. Left, labeled What you see: the Wikipedia article University of Colorado Boulder in Chrome, with its title, 38 languages, the Article and Talk tabs, coordinates, and an infobox with the university's seal. Right, labeled What requests gets: View Source of the same page, lines 1 to 29 of HTML, from DOCTYPE html and head to title University of Colorado Boulder - Wikipedia and on."}
...
Notice two things. First, as with the article, you included a custom `User-Agent` header identifying yourself and providing contact information. This is a norm of responsible web data collection that you will learn more about in @sec-ethics. Second, the `.json()` method converts the response body from a JSON string into a Python dictionary — a data structure you already know how to navigate. To see that structure before Python has it, paste the same URL into Chrome and tick **Pretty-print**, as @fig-pageviews-json shows.

![The pageviews API's answer to the chapter's URL in Chrome, with **Pretty-print** ticked, September 2026. The response is a dictionary whose one key, `items` ①, holds a list with one dictionary per day ②. Each day has a `timestamp` ③, written year, month, day, and hour, and its `views` ④: the two fields the plot below is drawn from.](images/ch-01/pageviews-json_annotated.png){#fig-pageviews-json .lightbox fig-alt="Chrome's JSON viewer with the Pretty-print box ticked, showing the response's start. After the opening brace comes the key items, opening a list, and the first day's dictionary: project en.wikipedia, article University_of_Colorado_Boulder, granularity daily, timestamp 2026010100, access all-access, agent all-agents, and views 657. The second day repeats the fields, with timestamp 2026010200 and views 761, and a third begins."}
...
- **`jupyter: command not found`** (or "not recognized" on Windows): Jupyter isn't installed in the active environment. Run `conda activate webdata`, then `pip install notebook`, and start `jupyter notebook` again.
- **`ModuleNotFoundError`**: A library is not installed in the Python your notebook runs. In a terminal where `webdata` is active (the prompt starts with `(webdata)`), run `pip install library-name`, then restart your Jupyter kernel. If the library is installed and the error remains, Jupyter was started from another environment: run `import sys; print(sys.executable)` in the notebook, and check that the path includes `webdata`.
...
- **Status code 403 (Forbidden)**: The server rejected your request, often because you did not include a `User-Agent` header; Wikipedia refuses `requests`' default. Send one with your email, as this chapter's requests do, and see @sec-ethics for more on headers.
