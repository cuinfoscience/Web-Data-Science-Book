...
This is **milestone M1**: headless captures, guards, retries, provenance, and
`check`. Still to come:

- **M2:** headed captures (DevTools, View Source, browser windows).
- **M3:** annotation, legibility checks, and composites.
- **M4:** evidence queries, copying figures into the course repo, and CI.

## Quick start

```bash
bash tools/shots/bootstrap.sh                     # once per container; safe to re-run
...
- **`bootstrap.sh`** does five things:
  ...
  - with `--headed`, adds the virtual display and input tools M2 needs.
...
- **`doctor`** answers whether capture works in this session. Do not reuse an earlier session's answer:
  - it checks the proxy, the browser, and a real capture of example.com;
...
- **Other modes:** `mode: headed` and `mode: composite` mark figures that wait for M2 and M3. `capture` skips them with a note.
- **Quoting:** quote any YAML value that contains ` #`, or everything after it becomes a comment.
...
`selftest` runs 20 offline checks of the guards, retries, `promote`, and `check` against a local web server. It needs the browser but no network.
