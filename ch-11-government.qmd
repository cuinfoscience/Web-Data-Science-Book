...
### Comparing CPI Across Locations
...
denver_params = {
    "series_id": "CUURS49BSA0",  # CPI, Denver-Aurora-Lakewood
    "api_key": fred_key,
    "file_type": "json",
...
national_params = {
    "series_id": "CPIAUCSL",     # CPI, All Urban Consumers (national)
    "api_key": fred_key,
    "file_type": "json",
    "units": "pc1",
    "observation_start": "2019-01-01"
...
df_denver = pd.DataFrame(requests.get(url, params=denver_params).json()["observations"])
df_national = pd.DataFrame(requests.get(url, params=national_params).json()["observations"])
...
response = requests.get("https://api.census.gov/data/2022/acs/acs5",
                         params=housing_params)
housing_data = response.json()

decades = ["2020+", "2010s", "2000s", "1990s", "1980s", "1970s"]
counts = [int(housing_data[1][i]) for i in range(2, 8)]
...
df_mortgage = pd.DataFrame(
    requests.get(url, params=mortgage_params).json()["observations"]
)
