...
::: {.callout-tip}
## Companion Notebook
Run this chapter's code as you read: open the [companion notebook](https://github.com/cuinfoscience/Web-Data-Science-Book/blob/main/notebooks/ch-13-ai-apis.ipynb) (see @sec-appendix-notebooks for all of them).
:::
...
### Structured Outputs: Letting the API Enforce the Format
...
This guided exercise is the chapter's take-home assignment. Work through it in the companion notebook, filling in each empty code cell, and submit the completed notebook. The steps build on one another, so do them in order — everything you need appears in this chapter or an earlier one. You will need an API key for one LLM provider; the whole exercise fits comfortably inside the small free-tier or trial budgets the chapter discusses.

You will run a miniature human–AI agreement study: classify a small corpus yourself, have a model classify the same items, and measure where you disagree. This is the validation habit that separates using LLMs from trusting them.

**Step 1 — Configure the client.** Load your API key from an environment variable (@sec-government) and create the client, following the chapter's setup. Confirm it works with a single trivial completion. Never paste the key into the notebook you submit.

```{python}
# Step 1: Load your API key from the environment, create the client, and run one test completion.
# Your code here
```

**Step 2 — Assemble a corpus.** Collect 15 short text items using any earlier chapter's technique — Bluesky posts (@sec-social), Wikipedia revision comments (@sec-wikipedia), RSS headlines (@sec-data-formats). Put them in a DataFrame with an `id` and a `text` column, and save it to CSV before spending any API budget on it (@sec-introduction).

```{python}
# Step 2: Collect 15 short texts with an earlier chapter's method, build a DataFrame, and save it to CSV.
# Your code here
```

**Step 3 — Design the task and label by hand.** Define a classification scheme with three or four mutually exclusive labels appropriate to your corpus (e.g., news / opinion / promotion / other). Write the labels and one-sentence definitions in a comment, then classify all 15 items yourself, adding a `human_label` column. Do this *before* the model sees anything — order matters for honesty.

```{python}
# Step 3: Define your labels in a comment and hand-label all 15 items into a human_label column BEFORE running the model.
# Your code here
```

**Step 4 — Prompt the model.** Write a classification prompt using the chapter's few-shot pattern: the label set, the definitions, one example per label, and the instruction to answer with a single label only. Loop it over the corpus into a `model_label` column, keeping temperature at 0 as the chapter recommends for classification.

```{python}
# Step 4: Write the few-shot prompt and classify all 15 items into a model_label column.
# Your code here
```

**Step 5 — Measure agreement.** Compute the agreement rate, and cross-tabulate `human_label` against `model_label` with `pd.crosstab()`. Pull out the disagreeing rows and read them.

```{python}
# Step 5: Compute the agreement rate, build the crosstab, and display every disagreement.
# Your code here
```

**Step 6 — Interpret.** In four to six sentences: What was your agreement rate, and are the disagreements random or systematic — does the model confuse one label pair in particular? On rereading, were any disagreements cases where the *model* was right? What would you change — in the labels, the definitions, or the prompt — before scaling this to a thousand items, and roughly what would that thousand-item run cost?
...
3. **Cross-model comparison.** Code 20 text samples manually according to a classification scheme of your choice. Have both GPT-4o-mini and Claude classify the same 20 samples. Compute the agreement rate between each model and your human labels, and between the two models. Where do the models disagree with each other, and what does that tell you about the task?

4. **Hierarchical clustering.** Generate embeddings for 30 Wikipedia article titles spanning three different topic areas (10 per topic). Use `scipy.cluster.hierarchy` to create a dendrogram. Do the embeddings cluster by topic? How does the clustering compare to your intuitive groupings?

5. **Graduate extension (INFO 5617).** Read @ziems2024can, which evaluates whether LLMs can serve as annotators for computational social science. Design and run a small validation study of your own: choose a classification task relevant to your research interests, classify 50 items with an LLM, and independently hand-code 25 of them before looking at the model's labels. Report your agreement rate (ideally alongside a chance-corrected measure such as Cohen's kappa), characterize any systematic patterns in the disagreements, and assess whether the paper's claims about zero-shot annotation hold for your task.
