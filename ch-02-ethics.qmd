...
# Identify yourself. The next section explains why this header matters;
# here it is already load-bearing, because Wikipedia refuses anonymous
# library defaults with a 403.
HEADERS = {
...
# Hand the parser text you fetched yourself, rather than calling
# rp.read(). The callout below explains what rp.read() does instead.
rp = RobotFileParser()
...
```{python}
# Step 2: Fetch Wikipedia's robots.txt and print the status code and
...
```{python}
# Step 3: Parse the file with rp.parse(text.splitlines()) and test
# can_fetch() on three different paths.
# Your code here
```
...
```{python}
# Step 4: Fetch robots.txt for two sites of your choice, sleeping between
# requests, and count lines, Disallow rules, and Allow rules for each.
# Your code here
```
