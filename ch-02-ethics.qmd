...
Reddit is the interesting case, and the shortest file. It is eight lines: a comment block, then `User-agent: *` followed by `Disallow: /`. Reddit currently restricts general crawling and directs researchers toward designated access programs. The comments point instead to Reddit's Public Content Policy and to a separate program for researchers. Reddit spent years as one of the most openly scrapable large platforms; the blanket denial is recent, and it arrived with the AI-licensing deals described in @sec-post-api.
