...
The two `print()` calls are how you check your work. The status code confirms the request completed, and echoing `response.request.headers["User-Agent"]` confirms that `requests` actually sent the string you set. Mistype the key as `"User Agent"` and the header is silently dropped, leaving the library default `python-requests/2.x` in its place. Note that the status code alone is not evidence the header worked: `example.com` answers anonymous requests with a `200` as well, so only the echoed header tells the two cases apart.
