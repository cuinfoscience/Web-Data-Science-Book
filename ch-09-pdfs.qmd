...
cleaned_text = re.sub(header_pattern, '', cleaned)

# 4. Section headings in ALL CAPS (at least 6 characters, start of line)
heading_pattern = r'^[A-Z][A-Z\s]{5,}$'
headings = re.findall(heading_pattern, cleaned, re.MULTILINE)
# Returns: ['PUBLIC COMMENT', 'CONSENT AGENDA', 'ADJOURNMENT', ...]
...
4. **Government PDF pipeline.** Choose a government agency that publishes PDF documents regularly (meeting minutes, inspection reports, court records, or budget documents). Download at least five PDFs from the same source, write a pipeline that extracts a consistent data element from each, build a DataFrame, and create a visualization showing how that element varies across documents.
