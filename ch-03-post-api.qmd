...
The exact exception name in the `evidence` column depends on how you reach the network: a direct connection to a host that has vanished usually raises `ConnectionError`, while a machine behind a proxy raises `ProxyError` instead. Either way the `status` is `None`, and that is the part that matters — no HTTP conversation happened at all. A browser shows the difference (@fig-dead-endpoints): Pushshift's refusal is a page Pushshift sends, while the page for CrowdTangle is Chrome's own, because nothing came back.

![Two of the retired endpoints in a browser, September 2026. Pushshift (top) answers: its 403 is one line of JSON, under Chrome's Pretty-print bar. CrowdTangle (bottom) doesn't, so the page is Chrome's own error page. It was captured through a proxy, so it names the failed tunnel, `ERR_TUNNEL_CONNECTION_FAILED`; on a direct connection, Chrome reports that it couldn't find the host's address.](images/ch-03/dead-endpoints.png){#fig-dead-endpoints .lightbox width="75%" fig-alt="Two screenshots, one above the other. Top, labeled api.pushshift.io answers: a 403, in JSON: Chrome's Pretty-print checkbox above one line of JSON, detail: Not authenticated. Bottom, labeled api.crowdtangle.com doesn't answer: the page is Chrome's own: a sad-page icon, This site can't be reached, a note that the webpage at https://api.crowdtangle.com/posts might be temporarily down, and ERR_TUNNEL_CONNECTION_FAILED."}
