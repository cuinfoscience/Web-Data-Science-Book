...
Technical systems embed political choices — who gets access, on what terms, and who decides. For a broader introduction to this idea, see *Missing Manual* [Chapter 8: Artifacts Have Politics](https://cuinfoscience.github.io/INFO-Missing-Manual/chapters/artifacts-have-politics.html).
:::
...
- **Reddit.** Reddit's API had been free since 2008. In 2023 the company announced pricing of $0.24 per 1,000 API calls, effective that July (@fig-reddit-api-pricing). The developer of Apollo, a popular third-party client, calculated the change would cost roughly $20 million a year and shut the app down; more than seven thousand subreddits went dark in protest. The same enforcement wave cut off Pushshift [@baumgartner2020pushshift], the archive that had underpinned hundreds of published Reddit studies. The research community lost not just future access but its accumulated observational infrastructure.

![Reddit's price in its own words: u/spez's post in r/reddit on 9 June 2023, as the Wayback Machine saved it on old.reddit.com a minute after it went up. Under "Premium Enterprise API / Third-party apps", the rate from 1 July 2023 is "$0.24 per 1K API calls", and Apollo, Reddit is Fun, and Sync "will close before pricing goes into effect." reddit.com's robots.txt now disallows every path, so the figure is the archive's copy.](images/ch-03/reddit-api-pricing.png){#fig-reddit-api-pricing .lightbox fig-alt="Two crops of an archived Reddit page, one above another. Top: the post Addressing the community about changes to our API, submitted 1 minute ago by spez. Bottom: Premium Enterprise API / Third-party apps. Effective July 1, 2023, the rate for apps that require higher usage limits is $0.24 per 1K API calls. Some apps such as Apollo, Reddit is Fun, and Sync will close before pricing goes into effect."}
...
The exact exception name in the `evidence` column depends on how you reach the network: a direct connection to a host that has vanished usually raises `ConnectionError`, while a machine behind a proxy raises `ProxyError` instead. Either way the `status` is `None`, and that is the part that matters — no HTTP conversation happened at all. A browser shows the difference (@fig-dead-endpoints): Pushshift's and Twitter's refusals are pages they send, while the page for CrowdTangle is Chrome's own, because nothing came back.

![The three retired endpoints in a browser, September 2026. Pushshift (top) and Twitter v1.1 (bottom) answer, each with one line of JSON under Chrome's Pretty-print bar: a 403, and a 400 with error 215. CrowdTangle (middle) doesn't, so the page is Chrome's own error page. It was captured through a proxy, so it names the failed tunnel, `ERR_TUNNEL_CONNECTION_FAILED`; on a direct connection, Chrome reports that it couldn't find the host's address.](images/ch-03/dead-endpoints.png){#fig-dead-endpoints .lightbox width="75%" fig-alt="Three screenshots, one above another. Top, labeled api.pushshift.io answers: a 403, in JSON: one line, detail: Not authenticated. Middle, labeled api.crowdtangle.com doesn't answer: the page is Chrome's own: This site can't be reached, the address https://api.crowdtangle.com/posts, and ERR_TUNNEL_CONNECTION_FAILED. Bottom, labeled api.twitter.com answers: a 400, error 215, in JSON: errors, code 215, message Bad Authentication data."}
...
Those timestamps are the shutdowns themselves, written in the archival record. Pushshift's closest capture is 25 May 2023 — six weeks before Reddit's July 2023 pricing took effect and the enforcement wave cut the archive off. CrowdTangle's is 14 August 2024, the day its own banner gave as its last (@fig-crowdtangle-last-capture). The archive kept visiting the address until December 2024, but every later capture records only a redirect: its copies of a site end when the site stops being a site.

![CrowdTangle's last capture in the Wayback Machine, 14 August 2024, shown in September 2026. Top: the archive's toolbar over the page, with the capture's date and the address's captures, 16 February 2012 to 12 December 2024; every one after this is a redirect. The toolbar is pinned over the page's own banner, which shows faintly through it. Bottom: the same capture without the toolbar (the archive's `if_` view), where the banner is legible.](images/ch-03/crowdtangle-last-capture.png){#fig-crowdtangle-last-capture .lightbox fig-alt="Two screenshots, one above another. Top, labeled The capture of 14 August 2024, under the Wayback Machine's toolbar: the address https://www.crowdtangle.com/, 3,892 captures, 16 Feb 2012 - 12 Dec 2024, and the date AUG 14 2024, over CrowdTangle's black menu bar. Bottom, labeled The same capture without the toolbar: a yellow banner, Important update: CrowdTangle will no longer be available after August 14, 2024, above the menu and the headline."}
