...
# Identify yourself. @sec-ethics explains why this header matters; here it is already load-bearing, because several of these hosts answer anonymous library defaults differently than they answer a named client.
...
You will run the chapter's access audit yourself, on today's web, and extend it to platforms you care about. Your numbers will differ from the book's; recording that difference, with a date, is the point.
...
**Step 2 — Probe one endpoint by hand.** Request the Wikipedia endpoint from the chapter's `TARGETS` with a 20-second timeout. Print the status code and the `content-type` header. Do not build the loop yet — make one request and look at it.

```{python}
# Step 2: Request one endpoint and print its status and content-type.
# Your code here
```

**Step 3 — Build the access matrix.** Write the `probe()` function from the chapter (or your own version) and run it over all eight `TARGETS`, collecting rows into a DataFrame. Remember the `Int64` dtype for the status column, and record today's date in a comment.

```{python}
# Step 3: Probe all eight endpoints and collect the results into a DataFrame.
# Record today's date -- your table is a snapshot, not a fact.
# Your code here
```

**Step 4 — Extend it.** Add two endpoints of your own, for platforms you might actually study, and re-run the matrix with ten rows. Classify each new row as open, gated, or closed.

```{python}
# Step 4: Add two endpoints you care about and rebuild the matrix.
# Your code here
```

**Step 5 — Attribute a refusal.** Pick one endpoint that returned a non-200 and diagnose it: print the `server` and `via` headers and the first 120 characters of the body. State in a comment whether the refusal came from the platform or from something between you and it.
...
**Step 6 — Autopsy a retired endpoint.** Run the chapter's `autopsy()` against one of the three retired endpoints and classify how it refuses: gone, gated, or repriced.

```{python}
# Step 6: Autopsy one retired endpoint and classify the manner of refusal.
# Your code here
```

**Step 7 — Interpret.** In four to six sentences: Where did your matrix differ from the book's, and what changed in between? Which of your two added platforms is more exposed to enclosure, and what evidence from the probe supports that? For the endpoint you autopsied, is there anyone left to negotiate with?
