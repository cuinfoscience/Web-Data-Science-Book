...
**Step 1 — Set up politely.** Define your `HEADERS` with a named `User-Agent` (@sec-ethics). Find an RSS feed from a news outlet you read — most sites serve one at `/rss` or `/feed` — and check the site's `robots.txt` permits fetching it.

```{python}
# Step 1: Define HEADERS and choose an RSS feed URL you have checked
# against the site's robots.txt.
# Your code here
```
...
```{python}
# Step 3: Parse with the "xml" parser, find_all the repeating items,
# and count them.
# Your code here
```
...
```{python}
# Step 4: Build a list of dicts (title, link, pubDate) and convert
# it to a DataFrame.
# Your code here
```
...
```{python}
# Step 5: Fetch the Open-Meteo forecast, parse the JSON, and print the
# top-level keys and the keys nested under "daily".
# Your code here
```
