...
Because the values are being stored in parallel lists, the different positionss must match to the same date. Before you makr the DataFrame, just make sure that the lists all have the same length and that the values are also in the same order.
