...
**Step 5 — Fetch nested JSON.** Retrieve the Boulder forecast from:
`https://api.open-meteo.com/v1/forecast?latitude=40.01&longitude=-105.27&daily=temperature_2m_max&timezone=America/Denver` 
Then parse it with `.json()`. Print the top-level keys, then the keys under `"daily"` — map the nesting before you extract anything.
