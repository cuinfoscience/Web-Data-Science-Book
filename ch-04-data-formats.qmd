...
Format choice is rarely arbitrary once you know what to look for. **JSON dominates anywhere a program is the primary reader.** REST APIs return it almost by default now — SOAP, the XML-based protocol REST displaced, requires wrapping every request in an XML namespace envelope that JSON has no equivalent for, which is part of why SOAP's use has been declining for years while REST and GraphQL have grown. JSON Web Tokens (JWTs), the credential your browser holds after you log into a site, are JSON underneath. Configuration files for modern developer tooling use several structured formats. package.json is JSON, while Kubernetes manifests are conventionally YAML, although JSON is also supported. The Kubernetes API commonly uses JSON for serialization, and most REST APIs use JSON request and response bodies because programs are doing the reading on both ends.
