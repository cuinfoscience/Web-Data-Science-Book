...
::: {.callout-tip}
## Companion Notebook
Run this chapter's code as you read: open the [companion notebook](https://github.com/cuinfoscience/Web-Data-Science-Book/blob/main/notebooks/ch-04-data-formats.ipynb) (see @sec-appendix-notebooks for all of them).
:::
...
Let us retrieve and parse actual Congressional member data. The Office of the Clerk of the U.S. House of Representatives publishes a membership roster at [clerk.house.gov](https://clerk.house.gov) in XML format. Its structure is what the synthetic example prepared you for: a root `<MemberData>` tag with a `publish-date` attribute, containing a `<members>` tag, containing one `<member>` per representative, each with the biographical details nested inside `<member-info>`. The real file adds two blocks we will ignore here: a `<title-info>` header before `<members>`, and a `<committee-assignments>` block in each `<member>`. You can check that shape before writing any code by opening the file's address in Chrome, which draws an XML file that has no stylesheet as a tree you can fold (@fig-house-xml-tree).

![The House roster in Chrome, September 2026: the root `<MemberData>` ① with its `publish-date`, `<members>` ②, and one `<member>` ③ per seat, whose biographical details sit in `<member-info>` ④. The second `<member>` ③ begins at the bottom. Chrome's triangles fold `<title-info>` and the first member's `<committee-assignments>`. Chrome's note above the tree, that the file has no style information, is cropped out.](images/ch-04/house-xml-tree_annotated.png){#fig-house-xml-tree .lightbox fig-alt="Screenshot of Chrome's tree view of MemberData.xml. Marker 1: the root tag, MemberData, publish-date September 2, 2026. Under a folded title-info, marker 2: members, and marker 3: the first member, statedistrict AK00. Marker 4: member-info, listing Nicholas Begich's names, bioguideID, party R, caucus R, state Alaska, district At Large, office, phone, and election and swearing-in dates. Marker 3 again: the second member."}

In Python, fetch the file with `requests` and parse it with BeautifulSoup's `"xml"` parser:
...
# Pass the bytes, not response.text: the server names no character set, so .text would guess wrong
...
Notice `response.content` rather than `response.text`. The Clerk's server labels the file `text/xml` without naming a character set, so `requests` falls back to ISO-8859-1, and `.text` would turn Linda T. Sánchez into "Linda T. SÃ¡nchez". Given the raw bytes, BeautifulSoup reads the encoding the file declares in its first line, UTF-8, and every name comes out right.
...
When you receive JSON data from the web (as a string), you need to parse it into Python objects. Python's built-in [`json` module](https://docs.python.org/3/library/json.html) does the work; its documentation lists every option these functions take:
...
The address in `url` is only half of the request. Opened on its own in a browser, `https://api.open-meteo.com/v1/forecast` returns an empty page, because the parameters are what ask for a forecast. `requests` adds them to the address for you, and `print(response.url)` shows the full URL it sent, which you can paste into a browser to see the same JSON (@fig-forecast-json).

![The chapter's Open-Meteo request, its full URL pasted into Chrome with **Pretty-print** ticked, September 2026. Beside the location, the top level holds two dictionaries: `daily_units` ① gives each list's unit, and `daily` ② holds the data. Its `time` ③ and `temperature_2m_max` ④ are parallel lists, one entry for each of seven days, so `data["daily"]["temperature_2m_max"][0]` is the first day's high. Pretty-print drops a whole number's `.0` (1635), which Python keeps (1635.0).](images/ch-04/forecast-json_annotated.png){#fig-forecast-json .lightbox width="78%" fig-alt="Screenshot of Chrome's JSON view with Pretty-print ticked: the forecast as indented JSON, from latitude, longitude, and timezone America/Denver to elevation 1635. Marker 1: daily_units, with time iso8601 and °C for both temperatures. Marker 2: daily. Marker 3: its time list, seven dates from 2026-09-24 to 2026-09-30. Marker 4: temperature_2m_max, seven numbers on one line, above temperature_2m_min."}
...
| The Changelog | `https://changelog.com/podcast/feed` — note: its `robots.txt` disallows all crawlers, so this feed fails Step 1's check |
...
- View the page source, not the rendered page (right-click the page and choose **View Page Source**, or press Ctrl + U; ⌘ + ⌥ + U on a Mac), and search it for `application/rss+xml`. The `<link>` tag with that `type` names the feed's address in its `href`, even when nothing on the page itself links to the feed (@fig-view-source-rss-link).
...
![PBS NewsHour's home page in View Source, searched with Chrome's find bar (Ctrl + F, or ⌘ + F on a Mac) for `application/rss+xml`, September 2026. The one match is on line 33, in a `<link rel="alternate">` whose `href` ① is the feed's address. The address bar shows the `view-source:` prefix; typed before any address, it opens that page's source.](images/ch-04/view-source-rss-link_annotated.png){#fig-view-source-rss-link .lightbox fig-alt="Screenshot of Chrome's toolbar over a View Source page. The address bar reads view-source:https://www.pbs.org/newshour/, and the find bar holds application/rss+xml, match 1 of 1. Under the end of a script, line 32 is a canonical link, and line 33 is a link with rel alternate, href https://www.pbs.org/newshour/feeds/rss/headlines, and type application/rss+xml, highlighted. Marker 1 is beside the href."}
:::
...
**Step 2 — Fetch and inspect the XML.** Retrieve the feed with `requests`. Print the status code and the first 500 characters. Identify the repeating element by eye — for RSS it is `<item>`. Chrome draws a feed served as XML as a tree you can fold (@fig-rss-feed-xml), and a feed served as `application/rss+xml`, such as PBS NewsHour's, as plain text.

![BBC News's science and environment feed in Chrome, September 2026. `<rss>` ① holds one `<channel>` ②: the feed's title, description, and link, then one `<item>` ③ per story, each with a title, description, link, `guid`, date, and thumbnail; the second `<item>` ③ begins at the bottom. Each `CDATA` wrapper marks text the parser takes as it is, and BeautifulSoup's `.text` returns what is inside. The channel's `<image>` and `<copyright>` are folded, and Chrome doesn't wrap CDATA, so the first description runs past the edge.](images/ch-04/rss-feed-xml_annotated.png){#fig-rss-feed-xml .lightbox fig-alt="Screenshot of Chrome's tree view of the feed. Marker 1: the rss root tag and its namespaces. Marker 2: channel, with title BBC News, description BBC News - Science & Environment, a link, and folded image and copyright. Marker 3: the first item, titled Elephants use medicinal plants to treat themselves, researchers find, with description, link, guid, pubDate of 24 September 2026, and thumbnail. Marker 3 again: the second item."}
...
- **Garbled accented letters, such as `SÃ¡nchez` for `Sánchez`**: `requests` decoded the body with the wrong character set, usually because the server's `Content-Type` header names none; `requests` then falls back to ISO-8859-1. Pass `response.content` to BeautifulSoup so the parser reads the encoding the document declares, or set `response.encoding = "utf-8"` before reading `response.text`.
