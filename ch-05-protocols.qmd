    ...
    for attempt in range(max_retries):
        ...
        if response.status_code == 429:
            ...
            if retry_after and retry_after.isdigit():
                wait_time = int(retry_after)  # the server told us how long to wait
            else:
...
Exponential backoff doubles the wait time with each retry, giving the server progressively more breathing room. Many APIs include a `Retry-After` header in their 429 responses that tells you exactly how long to wait — the function above checks for it with `response.headers.get("Retry-After")` before falling back to its own backoff schedule.
