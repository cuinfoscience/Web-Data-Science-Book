...
**Step 5 — Profile five URLs.** Run `diagnose_url()` on five different URLs that act differentl, one that works, one that gives 404, one that redirects from `http://` to `https://`, one API endpoint, and one site that stops tne automated access. For each write down the status code and explain what happened.
