...
```
::: {.callout-note}
## If Scapy Returns No Answers

On some systems, especially macOS, `scapy` may print `Received 0 packets, got 0 answers` if the notebook does not have permission to send or capture raw network packets. This does not necessarily mean that the destination is unreachable. As a simpler check on macOS or Linux, run `traceroute en.wikipedia.org` in the terminal. If `scapy` is required, you may need to run your environment with administrator privileges.
:::
...
- **`dns.resolver` raises `NoNameservers ... answered REFUSED`**: The kernel is querying campus resolvers (CU's are in the `128.138.x.x` range), which only answer recursive queries from on-campus addresses. `dnspython` reads DNS settings once per process, so a kernel started on campus or on VPN keeps them after you get home, even though `scutil --dns` shows your current ones. `requests` and your browser keep working, which makes it look specific to this exercise. Restart the kernel; if that doesn't fix it, build your own resolver with `r = dns.resolver.Resolver()`, set `r.nameservers = ["8.8.8.8", "1.1.1.1"]`, and call `r.resolve(...)` instead of `dns.resolver.resolve(...)`.
