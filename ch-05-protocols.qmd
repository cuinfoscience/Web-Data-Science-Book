...
::: {.callout-tip}
## Companion Notebook
Run this chapter's code as you read: open the [companion notebook](https://github.com/cuinfoscience/Web-Data-Science-Book/blob/main/notebooks/ch-05-protocols.ipynb) (see @sec-appendix-notebooks for all of them).
:::
...
The Inspector shows the HTML structure of the page as a tree, and it is the tool you will use most in this book. Chrome and Safari call this panel **Elements**; Firefox calls it **Inspector**. Whatever the name, the fastest way to open it on any specific piece of a page is to right-click the thing you care about — a headline, a table cell, an image — and choose **Inspect** (Chrome, Firefox) or **Inspect Element** (Safari). This jumps straight to that element in the tree (@fig-inspector-heading), rather than dropping you at the top of a page that might be thousands of lines long.

![The University of Colorado Boulder article's title ① highlighted by Chrome in September 2026, while the pointer rests on the title's node, selected in the Elements panel ②. The label gives the element's tag, `id`, classes, and size.](images/ch-05/inspector-heading_annotated.png){#fig-inspector-heading .lightbox fig-alt="Screenshot of Chrome at en.wikipedia.org/wiki/University_of_Colorado_Boulder with developer tools docked below the page. The article title is shaded blue under a tooltip reading h1#firstHeading.firstHeading.mw-first-heading, 552.3 by 39.59, and marker 1 sits at its end. Marker 2 points to the selected row in the Elements panel, an h1 element with id firstHeading, which sits inside a header element between a nav element and a div."}
...
Hover over any node in the tree and the browser highlights the corresponding region of the page — a fast way to confirm you have the right element before you write a selector for it. The reverse direction works too: click the small arrow-and-box icon in the DevTools toolbar (the **element picker**, sometimes called **inspect mode**; @fig-element-picker), then click anywhere on the rendered page, and the Inspector jumps to that element's node in the tree. This is usually faster than hunting through nested `<div>`s by eye, especially on a complex page.

![The element picker ① switched on, with the pointer over the article's infobox, September 2026. Chrome shades the element under the pointer, labels it with its tag, `id`, classes, and size, and highlights its node in the Elements tree ②. A click selects that node.](images/ch-05/element-picker_annotated.png){#fig-element-picker .lightbox fig-alt="Screenshot of Chrome on the same article with developer tools docked below. Marker 1 boxes the Select an element button at the left end of the toolbar, which is switched on. The article's infobox is shaded under a tooltip reading table#mwCQ.infobox.vcard, 309.76 by 1325.36, with its background, margin, padding, and accessibility name and role. Marker 2 points to the infobox's highlighted row in the Elements tree."}
...
Each row in the request list shows several columns at a glance: the request's **Name** (usually the URL's last path segment), its **Status** code, its **Type**, its **Size**, and its **Time** — how long that request took. The **Waterfall** column visualizes timing as a colored bar: a queuing delay, DNS lookup, connection setup, waiting for the server's first byte (often labeled TTFB, time-to-first-byte), and content download each get their own segment, so a request that is slow because the server is slow to respond looks visibly different from one that is slow because the response itself is simply large. Recent versions of Chrome hide the Waterfall column and draw a similar timeline above the list instead; to add the column, right-click any column heading and choose **Waterfall** (@fig-network-requests).

![The Network tab after reloading the article, September 2026. The first of 37 requests is the HTML document itself; the rest are the stylesheets, scripts, and images it asked for. On this first visit nothing was cached, so every row has a size and a time; reload again and many rows read `(memory cache)` or `(disk cache)`. Here the Waterfall column is switched on, and the Initiator column and the timeline Chrome normally draws above the list are hidden.](images/ch-05/network-requests.png){#fig-network-requests .lightbox fig-alt="Screenshot of Chrome on the same article with the Network tab docked below. Filter buttons read All, Fetch/XHR, Doc, CSS, JS, Font, Img, Media, Manifest, Socket, Wasm, and Other. The request list has columns Name, Status, Type, Size, Time, and Waterfall; its first row is the document University_of_Colorado_Boulder, status 200, 128 kB in 150 ms. The status bar reads 37 requests, 670 kB transferred."}

Click any individual request to open a detail pane with several sub-tabs (@fig-network-headers). **Headers** shows the request headers your browser sent (including `User-Agent`, `Accept`, and any `Cookie` header carrying session information) and the response headers the server sent back (including `Content-Type`, `Set-Cookie`, and caching directives). **Response** (Chrome/Safari) or the equivalent pane in Firefox shows the actual response body — for a Fetch/XHR request, this is usually where you confirm the exact JSON shape you will need to parse in Python, before writing a single line of code. **Timing** breaks the waterfall segments down into exact millisecond values.

![The request headers Chrome sent for the article, in the Headers tab, September 2026 (the response headers are above them, folded). The `Sec-Ch-Ua` rows ① are User-Agent Client Hints, reporting Chromium 154 on Linux; `User-Agent` ② is the identification this book's screenshots are taken with, where your browser would send a long `Mozilla/5.0 …` string. This capture used HTTP/1.1, through a proxy; over HTTP/2, header names are lowercase and `:authority` stands in for `Host`.](images/ch-05/network-headers_annotated.png){#fig-network-headers .lightbox fig-alt="Screenshot of the Network tab alone. The article's own request is selected at the top of the request list, and its Headers tab lists request headers from Accept to User-Agent, including Host, en.wikipedia.org. Marker 1, a brace, groups Sec-Ch-Ua, naming Chromium 154, Sec-Ch-Ua-Mobile, and Sec-Ch-Ua-Platform, Linux. Marker 2 points to the User-Agent value, Web Data Science/v1 brian.keegan@colorado.edu."}
...
- **macOS or Linux:** open a terminal and run `sudo jupyter notebook --allow-root`. The flag is not optional: without it, Jupyter detects that it is running as root, logs `Running as root is not recommended. Use --allow-root to bypass.`, and exits before the server ever starts. One gotcha worth knowing in advance: `sudo` resets your shell's `PATH` by default, so if `scapy` is installed inside a conda environment, a bare `sudo jupyter notebook` can launch a *different* Jupyter than the one in your environment — and fail with `ModuleNotFoundError: No module named 'scapy'` even though you installed it correctly. If that happens, be explicit about which Jupyter you mean: `sudo $(which jupyter) notebook --allow-root`.
...
(macOS and Linux; on Windows, or anywhere `dig` is not installed, `nslookup en.wikipedia.org` asks the same question.) An "A" record, short for address record, is the type of DNS record that gives a domain's IP address. A records hold only IPv4 addresses, so finding one also tells you the domain has an IPv4 address. Both return the same A record `dns.resolver` gave you, plus detail the Python call does not surface by default: which DNS server answered, and how long the result is cached for — its **TTL**, in seconds.
...
- **`sudo jupyter notebook` exits immediately**: Jupyter refuses to run as root unless you ask it to. It logs `Running as root is not recommended. Use --allow-root to bypass.` and stops, so no server starts and no notebook opens. Add the flag: `sudo jupyter notebook --allow-root`.
- **`scapy` fails after `sudo jupyter notebook --allow-root`**: `sudo` often resets `PATH`, launching a different Python than your conda environment. Use `sudo $(which jupyter) notebook --allow-root` instead.
