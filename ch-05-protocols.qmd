...
![The Network tab after reloading the article, September 2026. The first of 37 requests is the HTML document itself; the rest are the stylesheets, scripts, and images it asked for. On this first visit nothing was cached, so every row has a size and a time; reload again and many rows read `(memory cache)` or `(disk cache)`. The Waterfall column is switched on here, and the timeline Chrome normally draws above the list is hidden.](images/ch-05/network-requests.png){#fig-network-requests .lightbox fig-alt="Screenshot of Chrome on the same article with the Network tab docked below. Filter buttons read All, Fetch/XHR, Doc, CSS, JS, Font, Img, Media, Manifest, Socket, Wasm, and Other. The request list has columns Name, Status, Type, Initiator, Size, Time, and Waterfall; its first row is the document University_of_Colorado_Boulder, status 200, 128 kB in 132 ms. The status bar reads 37 requests, 670 kB transferred."}
