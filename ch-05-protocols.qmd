...
**Replicating `time.sleep()` between requests.** When you make several requests in a row, you can pause between them with Python's `time.sleep()`. This slows down your requests so you are not sending them to a server as quickly as your computer can. For example, `time.sleep(2)` waits two seconds before the next request:
...
This is a simple form of **rate limiting**: deliberately controlling how frequently you make requests. `wget` has a built-in equivalent, `--wait=2`, which waits two seconds between requests. `--random-wait` varies the length of those pauses rather than using the same delay every time.

`curl` does not have a direct equivalent to `--wait`; you would need to add the delay yourself in a shell loop:
