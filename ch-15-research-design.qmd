...
```{python}
# Step 1: State your research question, and name the construct it is
# really about. Write as comments, or convert this cell to Markdown.
```
...
```{python}
# Step 2: Name the source, the chapter technique you would use, and what
# the access rules permit.
```
...
```{python}
# Step 4: Describe the sample, the population it represents, and the gap
# between that and the population your question targets.
```
...
```{python}
# Step 5: Make the construct validity argument, name its weakest point,
# and propose one validation check.
```
...
```{python}
# Step 7: The ethical determination -- IRB or not, and the collection
# boundaries you would set.
```
