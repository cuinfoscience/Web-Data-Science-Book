...
- **Requests in code**: every live request in an example sends a User-Agent with the reader's address, `WebDataScience/1.0 (your-email@colorado.edu)`, as chapter 1 does, and never the library default, which Wikipedia refuses with a 403. The book's own tools send the course's User-Agent, `WebDataScience/1.0 (brian.keegan@colorado.edu)`, in the same form, since Wikimedia's API rate-limits any other form as unidentified, and robots.txt is read for it, except that a figure of an API's response, the request a chapter's code makes, is captured as an API client (`docs/decisions.md`).
...
- **Figures and screenshots**: images live in `images/ch-NN/`, each made from a recipe in `tools/shots/recipes/ch-NN.yml` and recorded in that folder's `provenance.json` (see `tools/shots/README.md`; run `tools/shots/run check` before a PR, and CI runs it again on any PR that changes `images/` or `tools/shots/`).
  - A screenshot is a real capture of a real page. Diagrams and renders are welcome, but never rebuild a real site's interface with invented content. The toolkit never signs in: a page behind a login is a person's screenshot, brought in with `tools/shots/run import` (`mode: hand`), which blacks out the account's details.
  ...
  - Before writing a recipe, read the "Field notes" in `tools/shots/README.md` for the kind of page you're capturing: robots.txt rules, including those addressed to AI agents; content types; Chrome's XML viewer and View Source; crops; and markers. When a capture teaches you something the next agent would otherwise find out again, add a note there.
  ...
  - No browser banners. A figure never shows Chrome for Testing's "only for automated testing" notice, or any other infobar. It takes 55 pixels and tells the reader nothing about the page. `tools/shots` launches Chrome with `--disable-infobars` and fails a headed take with an infobar above the page. A figure whose subject is the bar itself says so with `expect: {infobar: true}`; the only one is ch-08's window opened by Selenium. A capture made outside `tools/shots` needs the same switch.
