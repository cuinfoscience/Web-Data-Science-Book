...
**Updated 2026-09-25**, after textbook #148–#152 and #154–#165 and courses #55–#65 merged, with the third part of toolkit M4 that this note's pull request adds: `sync`, which copies figures into the course repo with a record of each. The course pull request paired with it holds the first records, for all 25 copies there. This note describes the present state. Rewrite it when a session stops or the state changes, and don't let it grow into a history. History lives in git, in [`decisions.md`](decisions.md), and in the AARs.
...
- **Screenshot toolkit, `tools/shots`.** Milestones M1–M3 are merged (#135–#137). Since the chapter 5 pilot:
  ...
  The selftest passes 127 of 127. `check` reports 0 errors and 0 warnings.
- **Chapters.**
...
- **Course repository.**
  ...
  - The course pull request paired with this note's records the 25 copies from the textbook: weeks 3, 5, 7, and 8's slides and weeks 1 and 4's handouts. `sync` reproduced every file byte for byte, so no image changed; each folder gains `shots.json` and a table in `IMAGES.md`. `AUTHORING.md` now names `sync` and `synced`.
...
4. **The rest of toolkit M4** ([plan](plans/2026-09-24-screenshot-toolkit.md), §8): `import`, for hand captures such as week 1's issue form, and the `shots-check` CI job (AAR P1-1). The engines, evidence queries, and `sync` are done; a copy made from now on should go through `sync`, so `synced` covers it. A figure whose caption or paragraph rests on a count should gain `evidence:` as it is next retaken; chapter 7's two are the pattern.
