...
The `HEADERS` constant deserves a comment before we go further. The [Wikimedia User-Agent policy](https://foundation.wikimedia.org/wiki/Policy:User-Agent_policy) asks every client to identify itself with a descriptive User-Agent string that includes contact information — anonymous or generic user agents may be throttled or blocked outright. Put your own email address in place of `your-email@colorado.edu`, keeping the form: a name and version, then your contact information in parentheses. Define it once as a module-level constant, and every function you write in this chapter can use it as a default.
...
# Revision timestamps parse as timezone-aware UTC (their ISO strings end in "Z"); drop the timezone so they align with the tz-naive pageview timestamps
...
**Step 1 — Choose an article and get its pageviews.** Pick an article on a topic that had at least one newsworthy moment in the past year. Define your `HEADERS` (@sec-ethics), then use the chapter's `get_pageviews()` (or your own version of it) to retrieve a full year of daily views. Build the DataFrame and plot the series.

```{python}
# Step 1: Retrieve a year of daily pageviews for your article, build the DataFrame, and plot it.
# Your code here
```

**Step 2 — Find the spike.** Identify the highest-traffic day with `idxmax()` (@sec-introduction showed you this) and print the date and view count. Note in a comment what real-world event you think explains it.
...
**Step 3 — Get the revision history.** Use the chapter's `get_revisions()` pattern — following continuation tokens so you page through the full history — and build a DataFrame of revisions with user and timestamp columns. Print the total count and the date range.
...
**Step 5 — Align edits with attention.** Resample or group the revisions by month, and plot monthly edit counts alongside (or above) the monthly pageview totals from Step 1. Do the two series move together around your spike?
...
**Step 6 — Map the neighborhood.** Use the chapter's `get_outlinks()` pattern to retrieve the article's outgoing wikilinks. Print the count and ten example targets.

```{python}
# Step 6: Retrieve the article's outgoing links; print the count and a sample.
# Your code here
```

**Step 7 — Interpret.** In four to six sentences: What does the alignment (or misalignment) of edits and views around your spike suggest about how Wikipedia responds to events? Is your article maintained by a dedicated few or a broad crowd, and why might that matter for how much you trust it? What kind of article do its outlinks suggest it is?
...
- **Rate limiting**: Wikimedia counts requests per minute across the Action API and the REST APIs, pageviews included: 200 a minute for a client whose User-Agent follows the policy, and 10 a minute for one that doesn't, shared with everyone else on its network address ([Wikimedia APIs/Rate limits](https://www.mediawiki.org/wiki/Wikimedia_APIs/Rate_limits); the limits are new in 2026 and may change). Past the limit, the answer is status 429. The Action API's etiquette guidelines also ask clients to make requests serially (wait for each response before sending the next) and to use the `maxlag` parameter for bulk work. The practical rule: serial requests, follow `continue` tokens, and always send an informative User-Agent per the Wikimedia User-Agent policy — without one, you may be throttled or blocked.
